...
=begin
Begin-Doc
Name: Local::MariaDBObject
Type: module
Description: object-based interface to MariaDB, child class derived from CommonDBObject
...
'options' can be passed in as a hash ref to set any other additional connection options
for the connect driver call.
...
sub SQL_OpenDatabase {
    ...
    my ( $user, $pass, $host, $port, $dsn, $options );
    $user = $info{"user"};
    $pass = $info{"passwd"};
    ...
    if ( !defined( $self->SQL_CurrentDatabase ) || $self->SQL_CurrentDatabase ne $database ) {
        ...
        # Since this module is starting new, baseline expectation is ssl by default
        if ( $info{ssl} ) {
            $dsn .= ";mariadb_ssl=1";
        }
        elsif ( exists($info{ssl} && !$info{ssl} ) {
            $dsn .= ";mariadb_ssl=0";
        }

        my $dbh = DBI->connect( $dsn, $user, $pass, $options ? $options : () );
