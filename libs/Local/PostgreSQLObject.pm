...
package Local::PostgreSQLObject;
use Exporter;
use strict;
...
sub SQL_OpenDatabase {
    ...
    if ( $info{nopasswd} ) {
        ...
        $pass = &AuthSrv_Fetch( user => $user, instance => "postgresql" );
    }
...
sub SQL_SerialNumber {
    ...
    # Technically not safe with sequences/etc - better to select from sequence explicitly
    $qry = "select lastval()";
