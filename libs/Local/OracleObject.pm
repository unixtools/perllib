...
$cid = $oradb->SQL_OpenQuery ($qry) or $oradb->SQL_Error && die "$qry";
while (($first, $last, $age) = $oradb->SQL_FetchRow (<b>$cid</b>))
{
        print "$first $last is $age years old\n";
}
$oradb->SQL_CloseQuery (<b>$cid</b>);



$qry = "insert into mytable (firstname, lastname, age) values (?, ?, ?)";
$cid = $oradb->SQL_OpenBoundQuery ($qry);
$oradb->SQL_ExecQuery ($cid, "John", "Jones", 99) or $oradb->SQL_Error;
$oradb->SQL_ExecQuery ($cid, "Jerry", "Smith", 47) or $oradb->SQL_Error;
$oradb->SQL_CloseQuery ($cid);


$oradb->SQL_CloseDatabase();
...
sub SQL_HTMLError {
    ...
    print "<P><H1>Oracle SQL Query Failed:</H1>\n";
    print "<PRE>$qry</PRE>\n";
...
sub SQL_Databases {
    return DBI->data_sources('Oracle');
}
